...
class flujograma(gr.top_block):
	def __init__(self):
		...
		nse = analog.noise_source_f(analog.GR_GAUSSIAN, 1)
		...
		e_fft=misbloques.e_vector_fft_square_ff(N)
		...
		v_avg=misbloques.vector_average_hob(N,100)
		#conexiones
		self.connect(src, (add, 0))
		self.connect(nse, (add, 1))
		#self.connect(add, str2vec, e_fft, vsnk)
 		self.connect(add, str2vec, e_fft,v_avg, vsnk)
		#graficar
